...
"""Interface to the the linear reservoir model."""
...
from .abcmodel_model import run_linres
from ..utils.metrics import mse
from ..utils.array_checks import check_for_negatives, validate_array_input


class linres(BaseModel):
    """Interface to the the linear reservoir model.

    This model implements a closed solution for the linear reservoir.

    If no model parameters are passed upon initialization, a random
    parameter set is generated.

    Args:
        params: (optional) Dictonary containing the model parameter k as a
            key/value pair.
    ...
    def get_random_params(self, num=1):
        """Generate a random parameter for the the linear resevoir.
        ...
        Returns:
            A dicttionary containing one key/value pair for the model parameter.

        """
        params = np.zeros(num, dtype=self._dtype)

        # sample parameter 'k' between the bounds [0,1]
        params['k'][:] = np.random.uniform(low=self._default_bounds['a'][0],
    ...
    def simulate(self, prec, initial_state=0, return_storage=False,
        ...
        externally defined linear reservoir function (see: linres_model.py).
        ...
        Args:
            ...
            params: (optional) Numpy array of parameter sets, that will be
                evaluated in parallel. If nothing is passed, the parameters,
                stored in the model object, will be used.

        Returns:
            An array with the simulated stream flow for each timestep and
            optionally an array with the simulated storage.
        ...
        # If no parameter is passed, prepare array w. params from attributes
        if params is None:
        ...
        # Call linear reservoir simulation function and return results
            if return_storage:
                qsim[:,i], storage[:,i] = run_linres(prec,
                                                     initial_state,
                                                     params[i])

            else:
                qsim[:,i], _ = run_linres(prec, initial_state, params[i])
...
def _loss(X, *args):
    ...
    qsim, _ = run_linres(prec, initial_state, params[0])
